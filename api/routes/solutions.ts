...
function renderServiceCards(serviceSlugs: string[]): string {
  return serviceSlugs.map((slug) => {
    ...
    return `
      <a href="/service/${service.slug}" class="solution-service-card">
...
app.get('/solutions/:slug', (c) => {
  const slug = c.req.param('slug') as SolutionKey
  const page = solutionMap.get(slug)
...
${pageScripts({ includeApp: false, includePages: true })}`, 404)
  ...
  return c.html(renderSolutionPage(page))
})
